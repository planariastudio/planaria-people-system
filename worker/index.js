...
function catchError(selector) {
  return operate(function(source2, subscriber) {
    ...
    var syncUnsub = false;
    var handledResult;
    innerSub = source2.subscribe(createOperatorSubscriber(subscriber, void 0, void 0, function(err2) {
      handledResult = innerFrom(selector(err2, catchError(selector)(source2)));
      if (innerSub) {
...
var Locator = class extends EventEmitter {
  ...
  #click(options) {
    ...
    return this._wait(options).pipe(this.operators.conditions([
      ...
      return from(handle.click(options)).pipe(catchError((err2) => {
  ...
  #hover(options) {
    ...
    return this._wait(options).pipe(this.operators.conditions([
      ...
      return from(handle.hover()).pipe(catchError((err2) => {
        void handle.dispose().catch(debugError);
        throw err2;
      }));
    }), this.operators.retryAndRaceWithSignalAndTimer(signal, cause));
  ...
  #scroll(options) {
    ...
    return this._wait(options).pipe(this.operators.conditions([
      ...
      }, options?.scrollTop, options?.scrollLeft)).pipe(catchError((err2) => {
        void handle.dispose().catch(debugError);
        throw err2;
      }));
    }), this.operators.retryAndRaceWithSignalAndTimer(signal, cause));
      ...
      __esDecorate3(this, _private_applyViewport_descriptor = { value: __setFunctionName(async function(client, viewportState) {
        ...
        await Promise.all([
          client.send("Emulation.setDeviceMetricsOverride", {
            ...
            if (err2.message.includes("Target does not support metrics override")) {
              debugError(err2);
              return;
            }
            throw err2;
...
var CdpHTTPResponse = class extends HTTPResponse {
  ...
  _resolveBody(err2) {
    if (err2) {
      return this.#bodyLoadedDeferred.reject(err2);
    }
  ...
  static async _create(client, target, defaultViewport) {
    ...
    if (defaultViewport) {
      try {
        ...
        if (isErrorLike(err2) && isTargetClosedError(err2)) {
          debugError(err2);
        } else {
          throw err2;
        }
...
async function clickupAttachPdf(env, taskId, filename, pdfBytes) {
  ...
  const res = await fetch(
    ...
    { method: "POST", headers: { Authorization: env.CLICKUP_TOKEN }, body: form }
  );
  if (!res.ok) throw new Error(`ClickUp attach failed: ${res.status} ${await res.text()}`);
  return res.json();
}
__name(clickupAttachPdf, "clickupAttachPdf");
// Comments are advisory (progress notes), so a failure must never fail the save
...
__name(clickupComment, "clickupComment");
async function clickupCreateFolder(env, name) {
  const res = await fetch(`https://api.clickup.com/api/v2/space/${env.CLICKUP_SPACE_ID}/folder`, {
    method: "POST",
    headers: { Authorization: env.CLICKUP_TOKEN, "Content-Type": "application/json" },
    body: JSON.stringify({ name })
  });
  if (!res.ok) throw new Error(`ClickUp create folder failed: ${res.status} ${await res.text()}`);
  return res.json();
}
...
async function clickupCreateList(env, folderId, name) {
  ...
  if (!res.ok) throw new Error(`ClickUp create list failed: ${res.status} ${await res.text()}`);
  return res.json();
}
...
// ClickUp rejects the call and the task simply stays in its home list.
// These RETURN whether it worked (they used to only catch network exceptions,
...
async function clickupCreateTaskInList(env, listId, name, opts = {}) {
  ...
  if (opts.start_date) payload.start_date = opts.start_date;
  ...
  if (Array.isArray(opts.assignees) && opts.assignees.length) payload.assignees = opts.assignees;
  const post = () => fetch(`https://api.clickup.com/api/v2/list/${listId}/task`, {
    method: "POST",
    headers: { Authorization: env.CLICKUP_TOKEN, "Content-Type": "application/json" },
...
async function clickupListStatuses(env, listId) {
  try {
    ...
    if (!res.ok) return [];
    const j = await res.json();
    return (j.statuses || []).map((s) => s.status);
  } catch (e) { return []; }
}
__name(clickupListStatuses, "clickupListStatuses");
// Best-effort real-status transition, layered ON TOP OF the existing task-name
// suffix convention (kept as-is, everywhere) rather than replacing it. We don't
// know each workspace list's exact configured status names up front -- they're
// looked up per call and matched case-insensitively by substring ("in progress"
// matches "In Progress", "IN PROGRESS", etc). If the list has no matching status
...
async function clickupSetStatus(env, taskId, listId, desiredSubstring) {
  try {
    ...
    const norm = (s) => String(s || "").trim().toLowerCase();
    const hit = statuses.find((s) => norm(s).includes(norm(desiredSubstring)));
    if (!hit) return false;
    await clickupUpdateTask(env, taskId, { status: hit });
    return true;
  } catch (e) { return false; }
}
__name(clickupSetStatus, "clickupSetStatus");
function quarterRange(q) {
  const m = /^Q([1-4])\s+(\d{4})$/.exec(String(q || "").trim().toUpperCase());
  if (!m) return null;
  const qn = +m[1], y = +m[2];
  const start = Date.UTC(y, (qn - 1) * 3, 1);
...
async function clickupResolveUserId(env, editorName) {
  try {
    if (!CLICKUP_MEMBER_CACHE) {
      ...
      if (!res.ok) return null;
      const j = await res.json();
      CLICKUP_MEMBER_CACHE = [];
      for (const t of (j.teams || [])) for (const m of (t.members || [])) if (m.user) CLICKUP_MEMBER_CACHE.push(m.user);
    }
    const norm = (s) => String(s || "").toLowerCase().replace(/\s+/g, " ").trim();
    const target = norm(editorName);
...
__name(clickupResolveUserId, "clickupResolveUserId");
// Resolves an editor's dedicated ClickUp list for a given report type, creating
// their Folder (+ KPI/Peer/PIP lists) on first use and caching IDs in Supabase.
...
// Maps an editor display name to their roster slug, so the folder cache keys on
// the stable id rather than a name that might be edited later.
async function editorSlug(env, name) {
  try {
    const rows = await sbSelect(env, "roster", `?name=eq.${encodeURIComponent(name)}&select=id`);
...
function kpiResultCardHtml(p) {
  ...
  *{box-sizing:border-box}body{margin:0;background:#fff;color:var(--ink);font-family:Inter,Arial,sans-serif;font-size:13px;line-height:1.5}
  ...
  .eyebrow{font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--kpi)}
...
async function handleConfig(env) {
  const [roster, levels, kpi_rubric, peer_rubric, pip_rubric, lists] = await Promise.all([
    ...
    sbSelect(env, "levels", "?select=*&order=sort_order"),
    sbSelect(env, "kpi_rubric", "?select=*"),
    sbSelect(env, "peer_rubric", "?select=*"),
    sbSelect(env, "pip_rubric", "?select=*"),
    sbSelect(env, "lists", "?select=*")
  ]);
  return json({
...
function pipReportHtml(p) {
  const verdictColor = p.verdict === "PASS" ? "#2f9e7e" : (p.verdict && p.verdict.startsWith("NOT MET")) ? "#e0603b" : "#475569";
  const gapRows = (p.gaps || []).map((g) => {
    const metCls = g.met === true ? "hi" : g.met === false ? "lo" : "mid";
    const metTxt = g.met === true ? "Met" : g.met === false ? "Not met" : "—";
    return `<tr>
      <td class="mn">${esc(g.competency)}</td>
      <td class="sc">${g.score_now != null ? esc(String(g.score_now)) : "—"}</td>
      <td class="sc">${g.score != null ? esc(String(g.score)) : "—"}</td>
      <td class="sc"><span class="chip ${metCls}">${metTxt}</span></td>
      <td class="nt">${esc(g.target || "")}</td>
      <td class="nt">${esc(g.evidence || "")}</td>
    </tr>`;
  }).join("");
  const actionRows = (p.actions || []).map((a) => `<tr>
      <td class="nt">${esc(a.editor || "")}</td>
      <td class="nt">${esc(a.company || "")}</td>
      <td class="sc">${esc(a.by || "")}</td>
    </tr>`).join("");
  const cpRows = (p.checkpoints || []).map((c) => {
    const otCls = c.on_track === "Yes" ? "hi" : c.on_track === "No" ? "lo" : "mid";
    return `<tr>
      <td class="sc">${c.n}</td>
      <td class="sc">${esc(c.date || "—")}</td>
      <td class="sc"><span class="chip ${otCls}">${esc(c.on_track || "—")}</span></td>
      <td class="nt">${esc(c.notes || "")}</td>
    </tr>`;
  }).join("");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
  :root{--ink:#17191d;--muted:#6b7280;--line:#e6e8eb;--pip:#b45309;--lo:#e0603b;--mid:#9ca3af;--hi:#2f9e7e;}
  *{box-sizing:border-box}body{margin:0;background:#fff;color:var(--ink);font-family:Inter,Arial,sans-serif;font-size:13px;line-height:1.5}
  .wrap{max-width:760px;margin:0 auto;padding:20px}
  .head{padding:0 0 16px;border-bottom:1px solid var(--line);display:flex;justify-content:space-between;align-items:flex-start}
  .eyebrow{font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--pip)}
  h1{font-size:20px;margin:6px 0 0}.meta{font-size:12px;color:var(--muted);margin-top:4px}
  .conf{font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--pip);border:1px solid var(--pip);border-radius:4px;padding:3px 7px}
  .verdict{margin:16px 0;padding:14px 18px;border-radius:10px;color:#fff;background:${verdictColor}}
  .verdict .vh{font-size:10px;text-transform:uppercase;letter-spacing:.1em;font-weight:700;opacity:.9}
  .verdict .vv{font-size:20px;font-weight:800;margin-top:2px}
  .section{padding:16px 0;border-bottom:1px solid var(--line)}
  .stitle{font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);margin-bottom:10px}
  ...
  th{text-align:left;font-size:9px;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);padding:5px 6px;border-bottom:1px solid var(--line)}
  td{padding:6px 6px;border-bottom:1px solid #f1f2f4;vertical-align:top}
  .sc{text-align:center;white-space:nowrap}.mn{font-weight:600}.nt{color:#374151;font-size:11px}
  .summary{font-size:12px;color:#374151;white-space:pre-wrap}
  </style></head><body><div class="wrap">
  <div class="head">
    <div><div class="eyebrow">Performance Improvement Plan</div><h1>${esc(p.editor)}</h1>
    <div class="meta">${esc(p.pip_id || "")} \xB7 ${esc(p.level || "")} \xB7 ${esc(p.severity || "")} \xB7 started ${esc(p.start_date || "")} \xB7 review ${esc(p.review_date || "")}</div>
    <div class="meta">Manager: ${esc((p.managers || []).join(" & ") || p.manager || "—")}</div></div>
    <div class="conf">Confidential</div>
  </div>
  <div class="verdict"><div class="vh">Verdict</div><div class="vv">${esc(p.verdict || "In progress")}</div></div>
  <div class="section"><div class="stitle">A \xB7 Gaps &amp; targets</div>
    <table><thead><tr><th>Competency</th><th>Now</th><th>Review</th><th>Met?</th><th>Target</th><th>Evidence</th></tr></thead>
    <tbody>${gapRows || '<tr><td colspan="6" class="nt">No competencies recorded yet.</td></tr>'}</tbody></table></div>
  <div class="section"><div class="stitle">B \xB7 Support actions</div>
    <table><thead><tr><th>What the editor will do</th><th>What the company provides</th><th>By when</th></tr></thead>
    <tbody>${actionRows || '<tr><td colspan="3" class="nt">No actions recorded yet.</td></tr>'}</tbody></table></div>
  <div class="section"><div class="stitle">C \xB7 Checkpoints</div>
    <table><thead><tr><th>#</th><th>Date</th><th>On track?</th><th>Notes / evidence reviewed</th></tr></thead>
    <tbody>${cpRows || '<tr><td colspan="4" class="nt">No checkpoints yet.</td></tr>'}</tbody></table></div>
  ${p.summary ? `<div class="section"><div class="stitle">Summary / rationale</div><div class="summary">${esc(p.summary)}</div></div>` : ""}
  </div></body></html>`;
}
__name(pipReportHtml, "pipReportHtml");
async function existingSubmission(env, table, rawFilters) {
  const q = Object.entries(rawFilters).map(([k, v]) => `${k}=eq.${encodeURIComponent(v)}`).join("&");
  const rows = await sbSelect(env, table, `?${q}&limit=1`);
  return rows.length > 0;
}
__name(existingSubmission, "existingSubmission");
async function pushToSheet(env, sheetName, row) {
  ...
  try {
    ...
    console.log("Sheet mirror push failed (non-fatal): " + e.message);
  }
}
...
async function handleKpiGet(env, id) {
  const rows = await sbSelect(env, "kpi_case", `?id=eq.${encodeURIComponent(id)}&select=*`);
  if (!rows.length) return json({ found: false });
  return json({ found: true, case: rows[0] });
}
__name(handleKpiGet, "handleKpiGet");
// Finds the editor's open (unfinalized) KPI case for a quarter, mirroring /pip/open.
async function handleKpiOpen(env, editorName, quarter) {
  if (!editorName) return err("missing editor");
  const slug = await editorSlug(env, editorName);
  const key = slug || editorName;
  let q = `?editor_id=eq.${encodeURIComponent(key)}`;
  if (quarter) q += `&quarter=eq.${encodeURIComponent(quarter)}`;
  q += `&order=created_at.desc&select=id,editor_locked,finalized,official_kpi`;
  const rows = await sbSelect(env, "kpi_case", q);
  const open = rows.find((r) => !r.finalized);
  if (!open) return json({ open: false, last: rows[0] || null });
  return json({ open: true, id: open.id, editor_locked: !!open.editor_locked });
}
__name(handleKpiOpen, "handleKpiOpen");
...
async function handleKpiEditor(env, body) {
  ...
  if (reminderTaskId) {
    ...
    try {
      // The pending self-review task lives in the To-do list (see
      // handleCreateKpiLinkTask) -- look statuses up from the list it's actually
      // in, since a list can override the space's status set.
      const statusListId = await resolveEditorList(env, slug, body.editor, "todo") || await resolveEditorList(env, slug, body.editor, "kpi");
      if (statusListId) await clickupSetStatus(env, reminderTaskId, statusListId, "in progress");
    } catch (e) {}
  }
...
_Full report: PDF attached. Focus areas are subtasks with next-quarter due dates._`;
}
__name(kpiTaskMd, "kpiTaskMd");
async function handleKpiFinalize(env, body) {
  ...
  for (const f of (payload.focus || [])) {
    ...
    try {
      await clickupCreateTaskInList(env, listId, `Focus \xB7 ${String(f.area).slice(0, 110)}`, {
        ...
        markdown_description: [f.why ? `**Why** ${f.why}` : null, f.action ? `**Agreed action** ${f.action}` : null, f.measure ? `**How we measure** ${f.measure}` : null].filter(Boolean).join("\n\n")
      });
    } catch (e) {}
  }
...
function peerTaskMd(agg, overallMean, slug) {
  const f2 = (v) => v == null ? "\u2014" : (+v).toFixed(2);
  const rows = (agg.values || []).map((v) => `| ${v.name} | ${f2(v.mean)} |`).join("\n");
  let trk = "";
  if (agg.track_reco) {
    trk = `\n**Track reads (senior)** IC ${agg.track_reco.ic} \xb7 Mgmt ${agg.track_reco.mg} \xb7 Unsure ${agg.track_reco.unsure}\n`;
  }
  return `**Peer overall ${f2(overallMean)}** \xb7 ${agg.n} rater${agg.n > 1 ? "s" : ""} pooled \xb7 ${agg.cycle}
...
function peerAggValues(rows) {
  const valueNames = rows.length ? Object.keys(rows[0].ratings) : [];
  const values = valueNames.map((name) => {
    const scores = rows.map((r) => r.ratings[name]).filter((x) => typeof x === "number").sort((a, b) => a - b);
    return { name, mean: scores.length ? scores.reduce((a, b) => a + b, 0) / scores.length : null, scores };
  });
  const track_reco = { mg: 0, ic: 0, unsure: 0 };
  rows.forEach((r) => { if (r.track_reco && track_reco[r.track_reco] !== void 0) track_reco[r.track_reco]++; });
...
async function handlePeerAggregate(env, targetSlug, cycle, key) {
  if (env.PEER_VIEW_KEY && key !== env.PEER_VIEW_KEY) return err("missing or wrong key", 403);
  if (!targetSlug || !cycle) return err("missing target/cycle");
  const roster = await sbSelect(env, "roster", `?id=eq.${encodeURIComponent(targetSlug)}&select=id,name,level`);
  const person = roster[0] || null;
  let rows = await sbSelect(env, "peer_response", `?target_id=eq.${encodeURIComponent(targetSlug)}&cycle=eq.${encodeURIComponent(cycle)}&select=ratings,track_reco,key_strength,constructive`);
...
async function handleCreateKpiLinkTask(env, body) {
  ...
  // HOME LIST = To-do, not KPI. While this is a thing the person still has to fill
  // in, it belongs beside their Peer prompt in one "what I owe" view. It previously
  // lived in the KPI list and was only MIRRORED into To-do, which meant that with
  // the Tasks-in-Multiple-Lists ClickApp switched off the mirror silently failed and
  // KPI never appeared in To-do at all. Creating it here needs no ClickApp; at filing
  // it moves to the KPI list (handleKpiFinalize), degrading to "stays in To-do,
  // marked Filed" if that ClickApp isn't available.
  const kpiListId = await resolveEditorList(env, person.id, person.name, "kpi");
  const todoListId = await resolveEditorList(env, person.id, person.name, "todo");
  const listId = todoListId || kpiListId;
  ...
  // Report WHERE it landed so "I clicked the button but see nothing in ClickUp"
  // is answerable from the UI instead of a guess.
...
// Read-only probe of every moving part (Supabase -> tables -> migrations ->
// config content -> ClickUp token/space -> Sheet webhook -> secrets), so
// "is the whole chain actually wired up?" is one click instead of a manual
// tour of four dashboards. Creates and mutates NOTHING.
async function handleSelfTest(env, key) {
  if (!checkAdminKey(env, key)) return err("Unauthorized", 401);
  const checks = [];
  ...
  add("Secrets · required", missingReq.length ? "fail" : "pass",
    missingReq.length ? `Missing: ${missingReq.join(", ")}` : `All ${required.length} present`);
  add("Secrets · optional", "info", optional.map((k) => `${k}${env[k] ? " ✓" : " —"}`).join("  "));

  // --- Supabase: connection + every table the system reads/writes ---
  const tables = ["roster", "levels", "kpi_rubric", "peer_rubric", "pip_rubric", "lists",
    "kpi_case", "pip_case", "peer_response", "peer_assignment", "peer_reminder", "clickup_map"];
  const badTables = [];
  for (const t of tables) {
    try { await sbSelect(env, t, "?select=*&limit=1"); }
    catch (e) { badTables.push(`${t} (${String(e.message).slice(0, 60)})`); }
  }
  ...
  try {
    const [roster, kpiR, peerR, pipR] = await Promise.all([
      sbSelect(env, "roster", "?active=eq.true&select=id"),
      sbSelect(env, "kpi_rubric", "?select=metric"),
      sbSelect(env, "peer_rubric", "?select=value"),
      sbSelect(env, "pip_rubric", "?select=*")
    ]);
    const empties = [];
    if (!roster.length) empties.push("roster");
    if (!kpiR.length) empties.push("kpi_rubric");
    if (!peerR.length) empties.push("peer_rubric");
    add("Config · content", empties.length ? "fail" : "pass",
      empties.length ? `EMPTY: ${empties.join(", ")} — re-sync the Sheet` : `${roster.length} active people · ${kpiR.length} KPI metrics · ${peerR.length} peer values · ${pipR.length} PIP rows`);
  } catch (e) { add("Config · content", "fail", String(e.message).slice(0, 120)); }

  // --- ClickUp: token valid, space reachable, folders visible ---
  let teamOk = false;
  try {
    const res = await fetch("https://api.clickup.com/api/v2/team", { headers: { Authorization: env.CLICKUP_TOKEN } });
    if (res.ok) {
      const j = await res.json();
      const members = (j.teams || []).reduce((a, t) => a + ((t.members || []).length), 0);
      teamOk = true;
      add("ClickUp · token", "pass", `Valid · ${(j.teams || []).length} workspace(s), ${members} members visible`);
    } else add("ClickUp · token", "fail", `HTTP ${res.status} — token rejected`);
  ...
  return json({ ok: failed === 0, failed, warned, checks, checked_at: (/* @__PURE__ */ new Date()).toISOString() });
}
__name(handleSelfTest, "handleSelfTest");
...
// Removing a case cascades: the Supabase row AND its ClickUp task go together, so
// the two systems can't drift into "tracker says gone, ClickUp still shows it".
// ClickUp deletes land in the workspace trash (restorable for ~30 days), which is
// what makes cascading acceptable for a button whose whole purpose is undoing a
// mis-entry. Supabase is deleted only after the ClickUp call resolves, and the
// ClickUp result is reported back so the UI can say what actually happened.
...
// was only ever "reserved" and never filled in has no task at all -- see
// handleCreatePipLinkTask -- in which case there's simply nothing to delete).
...
async function handleSubmitPeer(env, body) {
  ...
  const raterRows = await sbSelect(env, "roster", `?peer_token=eq.${encodeURIComponent(body._token)}&select=id,name`);
  if (!raterRows.length) return err("This rater link isn't recognized \u2014 ask your admin for a fresh peer-appraisal link.", 403);
  if (raterRows[0].name === body.target || raterRows[0].id === body.target_id) return err("You can't rate yourself \u2014 pick a different peer.", 400);
  const dup = await existingSubmission(env, "peer_response", { "rater": body._token, "cycle": body.cycle, "detail->>target": body.target });
  if (dup) return err(`You've already submitted a rating for ${body.target} this cycle (${body.cycle}). One rating per person per cycle.`, 409);
  const targetSlug = body.target_id || await editorSlug(env, body.target) || null;
  body.target_id = targetSlug;
  const priorRows = await sbSelect(env, "peer_response", `?detail->>target=eq.${encodeURIComponent(body.target)}&cycle=eq.${encodeURIComponent(body.cycle)}&select=detail`);
  let peerTaskId = null;
  for (const r of priorRows) { const t = r.detail && r.detail._clickup_task_id; if (t) { peerTaskId = t; break; } }
  await sbInsert(env, "peer_response", {
    target_id: targetSlug,
    cycle: body.cycle,
    rater: body._token || "unknown",
    // never returned in any read below
    ratings: body.ratings,
    key_strength: body.key_strength,
    constructive: body.constructive,
    overall: body.overall,
    track_reco: body.track_reco || null,
    detail: body,
  ...
  await pushToSheet(env, "Peer", { values: [
    ...
    ...PEER_VALUE_ORDER.map((v) => body.ratings[v] ?? ""),
    body.key_strength || "", body.constructive || "", body.overall, body.track_reco || ""
  ] });
  ...
  if (!peerTaskId) {
    const task = await clickupCreateTaskInList(env, peerList, `Peer Scorecard \xB7 ${body.target} \xB7 ${body.cycle}`, {
      ...
      markdown_description: peerTaskMd(agg, overallMean, targetSlug),
      start_date: qr ? qr.start : null,
      due_date: qr ? qr.end : null,
    ...
    try {
      await sbPatch(env, "peer_response",
        ...
        { detail: { ...body, _clickup_task_id: peerTaskId } });
    } catch (e) {}
  } else {
  ...
  // Guarded by the assigned-pool SIZE the final snapshot was fired for, not a plain
  // boolean -- a plain flag froze the "final" PDF forever at whatever the pool
  // looked like the first time it completed. If an admin assigns a new rater after
  // that (see #14), the pool grows, `n >= assigned.length` goes false again, and
  // once the newly-expanded pool completes a fresh final snapshot correctly fires
  // -- because the stored pool size no longer matches the current one.
  let allRatersIn = false, assignedCount = 0;
  try {
    ...
    allRatersIn = assignedCount > 0 && n >= assignedCount;
  } catch (e) {}
  // Self-correcting, tied to the same allRatersIn used for the final-PDF decision
  // above: if the pool later grows past a completed state, this naturally reverts
  // to "in progress" until the newly-expanded pool completes again.
  try { await clickupSetStatus(env, peerTaskId, peerList, allRatersIn ? "complete" : "in progress"); } catch (e) {}
  // Max, not first-match: priorRows has no guaranteed order (no ORDER BY on the
  // query), and after the pool has grown and re-completed more than once there
  // can be several rows carrying the flag -- an unordered .find() could latch
  // onto an old, smaller stored count forever and refire on every submission.
  ...
  const attachFinal = allRatersIn && !finalAlreadyFiledForThisPoolSize;
  if (isFirstScorecard || attachFinal) {
    ...
    if (attachFinal) {
      try {
        await sbPatch(env, "peer_response",
          `?rater=eq.${encodeURIComponent(body._token)}&cycle=eq.${encodeURIComponent(body.cycle)}&detail->>target=eq.${encodeURIComponent(body.target)}`,
          { detail: { ...body, _clickup_task_id: peerTaskId, _final_pdf: true, _final_pdf_for_assigned: assignedCount } });
      } catch (e) {}
    }
  } else {
...
__name(handlePipGet, "handlePipGet");
// Finds an editor's currently-open (not yet filed to ClickUp) PIP, if any.
// "Open" = the case exists and its detail._filed flag is not true.
async function handlePipOpen(env, editorName) {
  if (!editorName) return err("missing editor");
  const slug = await editorSlug(env, editorName);
  const key = slug || editorName;
  const rows = await sbSelect(env, "pip_case", `?editor_id=eq.${encodeURIComponent(key)}&order=created_at.desc&select=id,verdict,detail,created_at`);
  const open = rows.find((r) => !(r.detail && r.detail._filed));
  if (!open) return json({ open: false });
  return json({ open: true, id: open.id });
}
__name(handlePipOpen, "handlePipOpen");
// The PIP editor has no ClickUp presence by design (no personal link, no form
// access -- see handleCreatePipLinkTask) so they must never be an assignee here;
// being assigned a task they can't act on is exactly the "can view but can't
// fill it in" confusion reported. Managers -- the people actually running the
// case -- are the correct assignees.
async function pipManagerAssigneeIds(env, managers) {
  const ids = await Promise.all((managers || []).map((m) => clickupResolveUserId(env, m)));
...
async function handlePipUpdate(env, pipId, body) {
  ...
  const managerIds = await pipManagerAssigneeIds(env, body.managers);
  ...
  if (!taskId) {
    ...
    const task = await clickupCreateTaskInList(env, pipList, `PIP · ${body.editor} · ${pipId}`, {
      ...taskFields,
      assignees: managerIds.length ? managerIds : null,
    ...
    try { await clickupSetStatus(env, taskId, pipList, "in progress"); } catch (e) {}
  } else {
    // Every later save (checkpoints included) used to leave the description,
    // dates, and priority frozen at whatever the first save looked like -- an
    // edited severity, start date, or manager list never reached the task.
    await clickupUpdateTask(env, taskId, taskFields);
  }
  ...
  const actionSubtaskIds = await Promise.all(filledActions.map(async (a, i) => {
    ...
    try {
      const sub = await clickupCreateTaskInList(env, pipList, name, {
        parent: taskId, assignees: managerIds.length ? managerIds : null, priority: 3, due_date, markdown_description
      });
      return sub.id;
    } catch (e) { return null; }
  }));
  ...
  if (body._finalize && (body.verdict === "PASS" || body.verdict && body.verdict.startsWith("NOT MET"))) {
    ...
    try { await clickupSetStatus(env, taskId, pipList, "complete"); } catch (e) {}
  }
  ...
  const row = {
    ...
    duration_d: body.duration_d,
    start_date: body.start_date || null,
    review_date: body.review_date || null,
    final_review_date: body.final_review_date || null,
    result: body.result || null,
    verdict: body.verdict || null,
    detail: detailWithTask,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  await sbUpsert(env, "pip_case", row, "id");
  await pushToSheet(env, "PIP", { pip_id: pipId, values: [pipId, body.editor, body.level, (body.managers || []).join(" & "), body.severity, body.start_date, body.result || "", body.verdict || "", body.final_review_date || "", body.summary || "", taskId, (/* @__PURE__ */ new Date()).toISOString()] });
  return json({ ok: true, clickup_task_id: taskId });
}
...
async function sbReplaceAll(env, table, rows, pkCol) {
  ...
  const insRes = await fetch(`${env.SUPABASE_URL}/rest/v1/${table}`, {
    method: "POST",
    headers: sbHeaders(env, { "Content-Type": "application/json", Prefer: "return=minimal" }),
    body: JSON.stringify(rows)
  });
  if (!insRes.ok) throw new Error(`Config sync insert ${table} failed: ${insRes.status} ${await insRes.text()}`);
}
__name(sbReplaceAll, "sbReplaceAll");
async function handleConfigSync(env, body) {
  if (!body || !body.token || body.token !== env.CONFIG_SYNC_TOKEN) {
    return err("Unauthorized config sync \u2014 token missing or wrong.", 401);
  }
  // Server-side backstop: refuse to wipe a table to empty. A real edit never
  // sends an empty array for a table it means to keep; omit the key instead.
  for (const key of ["roster", "levels", "kpi_rubric", "peer_rubric", "pip_rubric", "lists"]) {
    if (Array.isArray(body[key]) && body[key].length === 0) {
      return err(`Refused: "${key}" arrived empty, which would wipe it live. To leave a table unchanged, omit it; this guard prevents accidental data loss.`, 400);
    }
  }
  const report = {};
  ...
  if (Array.isArray(body.levels)) {
    ...
    report.levels = body.levels.length;
  }
  if (Array.isArray(body.kpi_rubric)) {
    await sbReplaceAll(env, "kpi_rubric", body.kpi_rubric.map((r) => ({ id: r.id, category: r.category, metric: r.metric, levels: r.levels })), "id");
    report.kpi_rubric = body.kpi_rubric.length;
  }
  if (Array.isArray(body.peer_rubric)) {
    await sbReplaceAll(env, "peer_rubric", body.peer_rubric.map((r) => ({ id: r.id, cluster: r.cluster, value: r.value, levels: r.levels })), "id");
    report.peer_rubric = body.peer_rubric.length;
  }
  if (Array.isArray(body.pip_rubric)) {
    await sbReplaceAll(env, "pip_rubric", body.pip_rubric.map((r) => ({ id: r.id, competency: r.competency, kind: r.kind, levels: r.levels })), "id");
    report.pip_rubric = body.pip_rubric.length;
  }
  if (Array.isArray(body.lists)) {
    await sbReplaceAll(env, "lists", body.lists.map((l) => ({ key: l.key, value: l.value })), "key");
    report.lists = body.lists.length;
  }
  return json({ ok: true, synced: report });
}
__name(handleConfigSync, "handleConfigSync");
var index_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/selftest" && request.method === "GET") {
        return await handleSelfTest(env, url.searchParams.get("key"));
      }
